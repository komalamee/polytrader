...
const ALLOWED_STATUSES = new Set(['new', 'interested', 'saved', 'building', 'rejected']);
...
export function listIdeas({ status, sort = 'created_at', limit = 50, q = '' } = {}) {
  const db = getDb();
  ...
  const rows = db
    .prepare(`SELECT * FROM ideas ${where} ORDER BY ${orderBy} LIMIT @limit`)
...
export function createIdea(payload = {}) {
  ...
  db.prepare(`
    INSERT INTO ideas (
      ...
      status, battlestation_id, created_at, updated_at
    ) VALUES (
      @id, @title, @description, @source, @source_url, @source_date,
      @pain_signals, @reality_score, @edge_score, @edge_reason, @cycle_focus,
      @status, @battlestation_id, @created_at, @updated_at
